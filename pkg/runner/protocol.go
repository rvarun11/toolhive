...
func addCACertToTemplate(caCertPath string, templateData *templates.TemplateData) error {
	logger.Log.Debugf("Using custom CA certificate from: %s", caCertPath)
	...
	logger.Log.Debugf("Successfully validated and loaded CA certificate")
	return nil
}
...
func setupLocalBuildContext(packageName string) (*buildContext, error) {
	...
	logger.Log.Debugf("Using current working directory as build context: %s", currentDir)
				...
				if content, readErr := os.ReadFile(dockerfilePath); readErr == nil {
					if strings.Contains(string(content), "# Generated by ToolHive") {
						if err := os.Remove(dockerfilePath); err != nil {
							logger.Log.Debugf("Failed to remove temporary Dockerfile: %v", err)
						}
					}
				}
			}
...
func setupTempBuildContext() (*buildContext, error) {
	...
	logger.Log.Debugf("Using temporary directory as build context: %s", tempDir)
	...
	return &buildContext{
		...
		CleanupFunc: func() {
			if err := os.RemoveAll(tempDir); err != nil {
				logger.Log.Debugf("Failed to remove temporary directory: %v", err)
			}
		},
	}, nil
}
...
func writeDockerfile(dockerfilePath, dockerfileContent string, isLocalPath bool) error {
	if isLocalPath {
		...
		if _, err := os.Stat(dockerfilePath); err == nil {
			logger.Log.Infof("Dockerfile already exists at %s, using existing Dockerfile", dockerfilePath)
	...
	if isLocalPath {
		logger.Log.Debugf("Created temporary Dockerfile at %s", dockerfilePath)
	}
...
func writeCACertificate(buildContextDir, caCertContent string, isLocalPath bool) (func(), error) {
	...
	logger.Log.Debugf("Added CA certificate to build context: %s", caCertFilePath)
	...
	if isLocalPath {
		...
		cleanupFunc = func() {
			if err := os.Remove(caCertFilePath); err != nil {
				logger.Log.Debugf("Failed to remove temporary CA certificate: %v", err)
			}
		}
	} else {
