...
func (d *defaultManager) RunWorkloadDetached(ctx context.Context, runConfig *runner.RunConfig) error {
	...
	if runConfig.PermissionProfile != nil {
		...
		if err != nil {
			d.logger.Warnf("Warning: Failed to create permission profile file: %v", err)
		} else {
...
func removeClientConfigurations(containerName string, logger *zap.SugaredLogger) error {
	...
	configs, err := client.FindRegisteredClientConfigs(logger)
	if err != nil {
...
func (d *defaultManager) cleanupTempPermissionProfile(ctx context.Context, baseName string) error {
	...
	if r.Config.PermissionProfileNameOrPath != "" {
		if err := runner.CleanupTempPermissionProfile(r.Config.PermissionProfileNameOrPath, d.logger); err != nil {
